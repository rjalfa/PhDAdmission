...
public class AdminResultsController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{ 
			response.getWriter().println("<tr><td>" + a.getPersonalInformation().getEnrollmentNo() + "</td><td>" + a.getPersonalInformation().getApplicantName() + "</td>                                <td><a class=\"btn btn-sm btn-primary\" href=\"file://" + a.getEducationInformation().getAchievements().getCv().substring(0,a.getEducationInformation().getAchievements().getCv().length()>6?a.getEducationInformation().getAchievements().getCv().length()-6:0)+"Information.txt" + "\">DATA</a></td>                                <td><a class=\"btn btn-sm btn-primary\" href=\"file://" + a.getEducationInformation().getAchievements().getCv()  + "\">CV</a></td>                                <td><a class=\"btn btn-sm btn-primary\" href=\"file://"+ a.getEducationInformation().getAchievements().getSop() +"\">SOP</a></td>                            </tr>");
		}
